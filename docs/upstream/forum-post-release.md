...
| + RPS packet steering | **485** | **474** |

Nearly 3× on download. Two findings behind it:

1. Software flow offloading was off. Hardware offload needs NSS, so this is the
   whole budget. Note it is incompatible with SQM shaping.
2. `IRQ 22 (eth0)` had **1,357,683 interrupts on cpu0 and exactly zero on cpu1**
   — the conduit is single-queue, so one core did every packet while the other
   idled. Worth checking on any ipq50xx build.

**A warning on that second one:** the uci option
`network.globals.packet_steering` did **nothing** on my build. `netifd` contains
no `rps_cpus` code and no `packet_steering` string at all — I checked the binary.
Setting it is silently inert. I ship it as a hotplug script instead. If someone
knows which OpenWrt version actually implements that option, I would like to
know, because it looks configured and is not.
